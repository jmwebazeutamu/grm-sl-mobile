...
export function DecisionBar({ grievanceId, state, capabilities }: Props) {
  ...
  function onClose() {
    Alert.alert('Close this case?', 'This is the final state. It cannot be reopened later.', [
      { text: 'Cancel', style: 'cancel' },
  ...
  function onEscalate() {
    Alert.alert(
      ...
      'The case returns to In progress so more action can be taken before closing.',
      [
        { text: 'Cancel', style: 'cancel' },
        { text: 'Send back', onPress: () => escalate.mutate(undefined) },
      ],
    );
  }
